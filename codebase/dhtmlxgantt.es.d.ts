...
export interface GanttStatic {
   ...
   */
  addLinkLayer(
    func: AdditionalLinkLayer["LinkLayerRender"] | AdditionalLinkLayer["LinkLayerConfig"],
  ): string;
   ...
   */
  addTaskLayer(
    func: AdditionalTaskLayer["TaskLayerRender"] | AdditionalTaskLayer["TaskLayerConfig"],
  ): string;
   ...
   */
  attachEvent<T extends keyof GanttEventCallback>(
    event: T,
    handler: GanttEventCallback[T],
  ...
  event(
    ...
    options?: boolean | HandlerSettings,
  ): void;
   ...
   */
  getTaskBy(
    propertyName: string | GanttCallback,
   ...
   */
  isLinkAllowed(
    linkOrFrom: string | number | LinkForValidation,
    from_start?: boolean,
   ...
   */
  sort(
    field: string | ((task1: Task, task2: Task) => 1 | 0 | -1),
    desc?: boolean,
...
export interface CustomTypeRenderers {
  ...
  typeRenderer(
    task: Task,
    defaultRender?: AdditionalTaskLayer["TaskLayerRender"],
  ): HTMLElement | boolean | void | undefined;
}
...
export interface AdditionalTaskLayer {
  ...
  TaskLayerConfig: {
    ...
    renderer: {
      ...
      onrender?: (task: Task, node: HTMLElement, view?: any) => void;
      getRectangle?: (
        task: Task,
        view?: any,
        config?: GanttConfigOptions,
...
export interface AdditionalLinkLayer {
  ...
  LinkLayerConfig: {
    ...
    renderer: {
      ...
      onrender?: (link: Link, node: HTMLElement, view?: any) => void;
      getRectangle?: (
        link: Link,
        view?: any,
        config?: GanttConfigOptions,
...
export interface LightboxSection {
   ...
   * @param the Task object
   */
  template?(start_date: Date | number, end_date: Date | number, task: Task): string | number;
}
...
export interface LightboxControl {
   ...
   * @param the section's configuration object
   */
  set_value(node: HTMLElement, value: any, task: Task, section: LightboxSection): any;
   ...
   * @param the section's configuration object
   */
  get_value(node: HTMLElement, task: Task, section: LightboxSection): any;
...
export interface DatastoreMethods {
   ...
   */
  attachEvent<T extends keyof TreeDatastoreEvents>(
    event: T,
    handler: TreeDatastoreEvents[T],
    settings?: object,
  ): string;
...
export interface InlineEditor {
   ...
   */
  show(id: string | number, column: GridColumn, config: any, placeholder: HTMLElement): void;
   ...
   * @param the DOM element of the inline editor
   */
  set_value(value: any, id: string | number, column: GridColumn, node: HTMLElement): void;
   ...
   * @param the DOM element of the inline editor
   */
  get_value(id: string | number, column: GridColumn, node: HTMLElement): any;
   ...
   * @param the DOM element of the inline editor
   */
  is_changed?(value: any, id: string | number, column: GridColumn, node: HTMLElement): boolean;
   ...
   * @param the DOM element of the inline editor
   */
  is_valid?(value: any, id: string | number, column: GridColumn, node: HTMLElement): boolean;
   ...
   * @param the Task object
   */
  save?(id: string | number, column: GridColumn, node: HTMLElement): void;
...
export interface InlineEditorMethods {
   ...
   */
  setMapping(mapping: {
    init: (inlineEditors: InlineEditorMethods, grid: any) => void;
    onShow: (inlineEditors: InlineEditorMethods, node: HTMLElement, grid: any) => void;
    onHide: (inlineEditors: InlineEditorMethods, node: HTMLElement, grid: any) => void;
    destroy: () => void;
  }): void;
...
export interface InlineEditorEvents {
  onBeforeEditStart: (state: { id: number | string; columnName: string }) => any;

  onEditStart: (state: { id: number | string; columnName: string }) => any;
  ...
  onSave: (state: { id: number | string; columnName: string; oldValue: any; newValue: any }) => any;

  onEditEnd: (state: { id: number | string; columnName: string }) => any;
...
export interface KeyNav {
   ...
   */
  focus(config: { id: number | string; column: string; type: string }): void;
...
export interface Tooltips {
  ...
  tooltipFor(config: {
    selector: string;
    html: (event: Event, node: HTMLElement) => HTMLElement | string | number | void;
    global?: boolean;
  }): void;
